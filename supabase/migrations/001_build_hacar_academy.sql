-- HACAR ACADEMY MVP - SCHONE OPBOUW
-- LET OP: dit verwijdert alleen oude Academy-testtabellen en Academy-testgegevens.
-- Supabase Authentication-accounts in auth.users blijven bestaan.

begin;

-- Oude trigger eerst losmaken van auth.users.
drop trigger if exists on_auth_user_created on auth.users;

-- Oude en nieuwe Academy-tabellen veilig verwijderen.
drop table if exists public.monthly_competency_snapshots cascade;
drop table if exists public.monthly_item_snapshots cascade;
drop table if exists public.monthly_reports cascade;
drop table if exists public.student_item_notes cascade;
drop table if exists public.student_competency_assessment_history cascade;
drop table if exists public.student_item_assessment_history cascade;
drop table if exists public.student_module_notes cascade;
drop table if exists public.student_competency_progress cascade;
drop table if exists public.student_item_progress cascade;
...
-- Oude Academy-functies en typen verwijderen.
...
drop function if exists public.can_edit_student_progress(uuid) cascade;
drop function if exists public.create_monthly_report(uuid, date, text, text) cascade;
drop function if exists public.set_updated_at() cascade;
drop function if exists public.record_item_assessment_history() cascade;
drop function if exists public.record_competency_assessment_history() cascade;
...
create table if not exists public.student_item_assessment_history (
  id uuid primary key default gen_random_uuid(),
  student_id uuid not null references public.students(id) on delete cascade,
  module_item_id uuid not null references public.module_items(id) on delete cascade,
  score smallint not null check (score between 1 and 5),
...
create table if not exists public.student_competency_assessment_history (
  id uuid primary key default gen_random_uuid(),
  student_id uuid not null references public.students(id) on delete cascade,
  competency_id uuid not null references public.competencies(id) on delete cascade,
  score smallint not null check (score between 1 and 5),
...
create index if not exists idx_item_history_student_date on public.student_item_assessment_history(student_id, assessed_at desc);
create index if not exists idx_comp_history_student_date on public.student_competency_assessment_history(student_id, assessed_at desc);
...
create or replace function public.record_item_assessment_history()
returns trigger
language plpgsql
security definer
...
begin
  if tg_op = 'INSERT'
    ...
    insert into public.student_item_assessment_history (
      student_id, module_item_id, score, comment, assessed_by, assessed_at
    ) values (
      new.student_id, new.module_item_id, new.score, new.comment, new.updated_by, now()
    );
  end if;
  return new;
end;
...
create or replace function public.record_competency_assessment_history()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if tg_op = 'INSERT'
     or old.score is distinct from new.score
     or old.comment is distinct from new.comment then
    insert into public.student_competency_assessment_history (
      student_id, competency_id, score, comment, assessed_by, assessed_at
    ) values (
      new.student_id, new.competency_id, new.score, new.comment, new.updated_by, now()
    );
  end if;
  return new;
end;
...
DROP TRIGGER IF EXISTS item_progress_history ON public.student_item_progress;
create trigger item_progress_history
after insert or update on public.student_item_progress
for each row execute function public.record_item_assessment_history();

DROP TRIGGER IF EXISTS competency_progress_history ON public.student_competency_progress;
create trigger competency_progress_history
after insert or update on public.student_competency_progress
for each row execute function public.record_competency_assessment_history();
...
alter table public.student_item_assessment_history enable row level security;
alter table public.student_competency_assessment_history enable row level security;
...
drop policy if exists item_history_read on public.student_item_assessment_history;
create policy item_history_read on public.student_item_assessment_history for select to authenticated using (public.can_view_student(student_id));

drop policy if exists competency_history_read on public.student_competency_assessment_history;
create policy competency_history_read on public.student_competency_assessment_history for select to authenticated using (public.can_view_student(student_id));
...
grant select on public.profiles, public.students, public.student_assignments, public.modules, public.module_items, public.competencies, public.student_item_progress, public.student_competency_progress, public.student_item_assessment_history, public.student_competency_assessment_history, public.student_module_notes, public.student_item_notes, public.monthly_reports, public.monthly_item_snapshots, public.monthly_competency_snapshots to authenticated;
...
-- Laat de Supabase API de nieuwe tabelstructuur direct opnieuw inlezen.
